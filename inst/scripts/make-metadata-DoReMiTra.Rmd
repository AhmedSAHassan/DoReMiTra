...
generate_experimenthub_metadata <- function(
  ...
  metadata_entries <- lapply(names(se_list), function(se_name) {
    ...
    tissue <- if ("Tissue" %in% colnames(col_data)) {
      paste(unique(as.character(col_data$Tissue)), collapse = "; ")
    } else {
      NA
    }
...
metadata$SourceUrl[36] <- "https://github.com/AhmedSAHassan/Phybion_ShortReads_Analysis"
