...
fetch_geo_datasets <- function(
  ...
  for (i in seq_len(nrow(df))) {
    ...
    message(sprintf("📦 Fetching %s (%s)...", accession, treatment))
    ...
    tryCatch(
      {
        ...
        if (inherits(gse, "ExpressionSet")) {
          ...
          warning(sprintf("⚠️ %s did not return a valid ExpressionSet.", accession))
          failed <- c(failed, accession)
        }
      },
      error = function(e) {
        warning(sprintf("❌ Failed to fetch %s: %s", accession, e$message))
        failed <- c(failed, accession)
      }
    )
  }
  ...
  if (length(failed) > 0) {
    message("⚠️ These accession numbers failed to fetch:")
    print(unique(failed))
  }
}
```
...
```{r}
extract_and_save_SE_parts <- function(prefix = "X_ray_GSE", output_dir = ".") {
  ...
  for (obj_name in se_objects) {
    ...
    # Determine gene symbol column (GENE_SYMBOL or Gene.Symbol)
    gene_symbol_col <- NULL
    if ("GENE_SYMBOL" %in% colnames(rd)) {
      gene_symbol_col <- "GENE_SYMBOL"
    } else if ("Gene.Symbol" %in% colnames(rd)) {
    ...
    if (!is.null(gene_symbol_col)) {
      ...
      # Aggregate rowData by gene symbol, take first probe’s metadata per gene
      ...
      warning(sprintf("Neither GENE_SYMBOL nor Gene.Symbol found in rowData for %s. Keeping original counts and rowData.", obj_name))
    }
    ...
    message(sprintf("✅ Saved averaged components of %s to RDS files.", obj_name))
  }
}
```

```{r}
extract_and_save_SE_parts(prefix = "X_ray_GSE", output_dir = "inst/extdata/X-ray")
```

# Creating a SE object for each dataset

```{r}
assemble_SE_from_RDS <- function(input_dir = "inst/extdata/X-ray", prefix = "X_ray_GSE") {
...
```{r}
Xray_se_objects <- assemble_SE_from_RDS(input_dir = "inst/extdata/X-ray", prefix = "X_ray_GSE")
```
...
save_SE_list_as_RDS <- function(se_list, output_dir = ".", prefix = "SE_object") {
  ...
  for (i in seq_along(se_list)) {
    ...
    message(sprintf("✅ Saved %s as a single RDS file.", obj_name))
  }
}
```

```{r}
save_SE_list_as_RDS(Neutron_se_objects, output_dir = "inst/extdata/SE_objects", prefix = "Neutron_GSE")
```
...
```{r}
save_SE_list_as_RDS(Xray_se_objects, output_dir = "inst/extdata/SE_objects/Xray", prefix = "Xray_GSE")
```
